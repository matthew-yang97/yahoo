...
start_date = ['2020-09-14', '2020-09-14', '2020-09-14', '2020-09-14', '2020-09-14',
              '2020-09-14', '2020-08-31', '2020-09-14', '2020-09-14', '2020-11-02']
start_index = [36, 36, 36, 36, 36,
               36, 34, 36, 36, 43]
start_index2 = [257, 257, 257, 257, 257,
                257, 243, 257, 257, 306]
period_s = [357, 357, 357, 357, 357,
            357, 371, 357, 357, 308]
...
def max_pred(df, filename_pred, city):
    ...
    # from start_date to 2021-09-05
    dfsub = df[start_index[city]:87]
...
def diff_calc(max_file, city, rate_mean):
    ...
    date_all = pd.date_range(start=start_date[city], periods=period_s[city], freq='1D')
    gt = dfPop[start_index2[city]:614][names[city]].values.tolist()
    df_diff = pd.DataFrame({'y': gt})
    pred = []
    for w in range(int(period_s[city]/7)):
...
dfResult.to_csv('base98_2.csv', index_label='date')
